...
def isolated_reference_indices(reference,rel_tol):
    r=np.asarray(reference).ravel();out=[]
    for j,z in enumerate(r):
        sep=np.abs(r-z);sep[j]=np.inf
        rel=float(np.min(sep)/max(1.0,abs(z))) if len(r)>1 else np.inf
        if rel>rel_tol:out.append(j)
    return set(out)
...
def main():
    ...
    if x.transient_tolerances and Path(x.transient_tolerances).exists():
        t=json.loads(Path(x.transient_tolerances).read_text());tr=run_foundation_time_response(foundation_time_model(),scalar(m,'time_rotor_speed'),scalar(m,'time_dt'),int(scalar(m,'time_npts')),nr=int(scalar(m,'time_nr')));te=max_abs(tr.response,m['time_response']);tf=max_abs(tr.forcing,vec(m,'time_force'));ru=run_runup(runup_model(),vec(m,'runup_alpha'),[scalar(m,'runup_t0'),scalar(m,'runup_tf')],nr=int(scalar(m,'runup_nr')),max_points=300000);rt=vec(m,'runup_time');interp=np.vstack([np.interp(rt,ru.time_s,row) for row in ru.response]);re=max_abs(interp,m['runup_response']);ok=te<=t['time_fdn_max_abs_m'] and re<=t['runup_max_abs_m'] and tf<=t.get('time_fdn_force_max_abs',1e-12);g['G10']={'status':'PASS' if ok else 'FAIL','detail':f'time={te:.3e},runup={re:.3e},force={tf:.3e}'}
...
if __name__=='__main__':raise SystemExit(main())    # G7 stationary modal: eigenvalue/frequency gates apply to all modes.
    # Individual-vector MAC/kappa apply only to isolated modes because a
    # degenerate eigenspace has no unique vector basis.
    speeds=vec(m,'st_modal_speeds');er=[];macs=[];freqerrs=[];kaperrs=[];nskip=0
    ref_eig=np.asarray(m['st_modal_eig']);ref_vec=np.asarray(m['st_modal_vec']);ref_k=np.asarray(m['st_modal_kappa'])
    if ref_eig.ndim==1:ref_eig=ref_eig[:,None]
    for q,w in enumerate(speeds):
        r=run_modal(sm,float(w),with_eigenvectors=True,with_kappa=True);re=ref_eig[:,q];mapping=match_modes(r.eigenvalues,re);er.append(eigenvalue_max_rel(r.eigenvalues,re,mapping));freqerrs.append(max(abs(abs(r.eigenvalues[i])/(2*np.pi)-abs(re[j])/(2*np.pi))/max(1,abs(re[j])/(2*np.pi)) for i,j in mapping))
        isolated=isolated_reference_indices(re,GATES['eig_rel']);pairs=[(i,j) for i,j in mapping if j in isolated];nskip += len(mapping)-len(pairs)
        if pairs:
            rv=ref_vec[:,:,q] if ref_vec.ndim==3 else ref_vec;rk=ref_k[:,:,q] if ref_k.ndim==3 else ref_k
            for i,j in pairs:
                den=np.vdot(r.eigenvectors[:,i],r.eigenvectors[:,i]).real*np.vdot(rv[:,j],rv[:,j]).real
                macs.append(1.0 if den<=0 else float(abs(np.vdot(r.eigenvectors[:,i],rv[:,j]))**2/den))
                kaperrs.append(float(np.max(np.abs(r.kappa[:,i]-rk[:,j]))))
    min_mac=min(macs) if macs else 1.0;max_kappa=max(kaperrs) if kaperrs else 0.0
    ok=max(er)<=GATES['eig_rel'] and max(freqerrs)<=GATES['freq_rel'] and min_mac>=GATES['mac_min'] and max_kappa<=GATES['kappa_absrel']
    gates['G7']={'status':'PASS' if ok else 'FAIL','detail':f'eig={max(er):.3e},freq={max(freqerrs):.3e},isolated_MACmin={min_mac:.9f},isolated_kappa_abs={max_kappa:.3e},nonisolated_skipped={nskip}'}
    # G10 transient: compare exactly the DOFs/time support used to freeze the
    # immutable tolerance policy. Never apply a subset-derived threshold to all DOFs.
    if a.transient_tolerances and Path(a.transient_tolerances).exists():
        tol=json.loads(Path(a.transient_tolerances).read_text());refdir=Path(__file__).resolve().parents[1]/'baseline'/'transient';tp=refdir/'time_fdn_source_reference.npz';rp=refdir/'runup_source_reference.npz'
        if not tp.exists() or not rp.exists():
            gates['G10']={'status':'BLOCKED','detail':'source-derived transient reference files missing'}
        elif tol.get('time_source_reference_sha256')!=sha256(tp) or tol.get('runup_source_reference_sha256')!=sha256(rp):
            gates['G10']={'status':'BLOCKED','detail':'transient reference hash mismatch versus frozen tolerance policy'}
        else:
            tref=np.load(tp);rref=np.load(rp);tdofs=np.asarray(tref['response_dofs'],dtype=int);rdofs=np.asarray(rref['response_dofs'],dtype=int)
            tm=foundation_time_model();tr=run_foundation_time_response(tm,scalar(m,'time_rotor_speed'),scalar(m,'time_dt'),int(scalar(m,'time_npts')),nr=int(scalar(m,'time_nr')),rtol=1e-3,atol=1e-6);te=max_abs(tr.response[tdofs,:],np.asarray(m['time_response'])[tdofs,:]);tf=max_abs(tr.forcing,vec(m,'time_force'))
            ru=runup_model();rr=run_runup(ru,vec(m,'runup_alpha'),[scalar(m,'runup_t0'),scalar(m,'runup_tf')],nr=int(scalar(m,'runup_nr')),rtol=1e-3,atol=1e-6,max_points=300000);rt=np.asarray(rref['time']).ravel();ot=vec(m,'runup_time');orx=np.asarray(m['runup_response']);finterp=np.vstack([np.interp(rt,rr.time_s,rr.response[i,:]) for i in rdofs]);ointerp=np.vstack([np.interp(rt,ot,orx[i,:]) for i in rdofs]);re=max_abs(finterp,ointerp)
            ok=te<=tol['time_fdn_max_abs_m'] and re<=tol['runup_max_abs_m'] and tf<=tol.get('time_fdn_force_max_abs',1e-12);gates['G10']={'status':'PASS' if ok else 'FAIL','detail':f'time_fdn_abs={te:.3e}/{tol["time_fdn_max_abs_m"]:.3e},runup_abs={re:.3e}/{tol["runup_max_abs_m"]:.3e},force={tf:.3e},time_dofs={tdofs.tolist()},runup_dofs={rdofs.tolist()}'}
    else:gates['G10']={'status':'BLOCKED','detail':'transient MATLAB tolerance policy not frozen yet'}
import argparse,json,hashlib
from pathlib import Path
import numpy as np
from scipy.io import loadmat
