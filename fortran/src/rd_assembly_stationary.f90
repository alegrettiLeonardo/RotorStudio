...
contains
  ...
  subroutine assemble_bearings(nnode,nbear,bear,speed,Mb,Cb,Kb,is_zero,status)
    ...
    integer(ik),intent(out)::status
    integer::i,j,k,t,n,d(4); real(rk)::kl(4,4),cl(4,4),ml(4,4)
    ...
    do i=1,nbear
      ...
      d=[4*n-3,4*n-2,4*n-1,4*n];kl=0;cl=0;ml=0
      select case(t)
      ...
      case(5);kl(1:2,1:2)=reshape([bear(3,i),bear(5,i),bear(4,i),bear(6,i)],[2,2]);cl(1:2,1:2)=reshape([bear(7,i),bear(9,i),bear(8,i),bear(10,i)],[2,2])
      case(6)
        do j=1,4;do k=1,4;kl(j,k)=bear(2+(j-1)*4+k,i);cl(j,k)=bear(18+(j-1)*4+k,i);enddo;enddo
      case(7)
        call short_bearing_coefficients(bear(:,i),speed,kl,cl,status);if(status/=RD_OK)return
      case(8)
  ...
  subroutine short_bearing_coefficients(row,speed,K,C,status)
    real(rk),intent(in)::row(34),speed;real(rk),intent(out)::K(4,4),C(4,4);integer(ik),intent(out)::status
    real(rk)::F,D,L,cclr,eta,H,coef(0:4),Acomp(4,4),wr(4),wi(4),vr(4,4),n2,n,q1,q2,q3,p2,de,a(2,2),b(2,2)
    integer::i,nroot;integer(ik)::st
    K=0;C=0;status=RD_OK
    ...
    coef=[1._rk,-(4+acos(-1._rk)**2/H),6-(16-acos(-1._rk)**2)/H,-4._rk,1._rk]
    Acomp=0;Acomp(2,1)=1;Acomp(3,2)=1;Acomp(4,3)=1
    Acomp(1,:)=[-coef(3)/coef(4),-coef(2)/coef(4),-coef(1)/coef(4),-coef(0)/coef(4)]
    ...
    enddo
    if(nroot==0)n2=.5_rk
    n=sqrt(n2);p2=acos(-1._rk)**2;q1=1-n2;q2=1+n2;q3=1+2*n2;de=(p2*q1+16*n2)**1.5_rk
  ...
  subroutine seal_coefficients(row,speed,M,C,K,status)
    ...
    I2=0;I2(1,1)=1;I2(2,2)=1;J=reshape([0._rk,-1._rk,1._rk,0._rk],[2,2],order=[2,1])
