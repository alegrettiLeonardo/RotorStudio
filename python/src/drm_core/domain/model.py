...
@dataclass(frozen=True)
class Seal(Bearing):
    """Typed bearing-compatible representation of legacy bearing type 8."""
    @staticmethod
    def from_legacy(node:int,properties)->"Seal":
        return Seal(8,node,tuple(map(float,properties)))
...
class RotorModel:
    ...
    def from_legacy_arrays(cls,node,shaft,disc,bearing,force=None,bend=None,rotors=None)->"RotorModel":
        ...
        bearings=[]
        for r in bearing:
            t=int(round(r[0])); props=tuple(map(float,r[2:]))
            bearings.append(Seal(t,int(r[1]),props) if t==8 else Bearing(t,int(r[1]),props))
