...
def run_coaxial_modal(model,speed_rad_s,library_path=None):
    ...
    return CoaxialModalResult(speed_rad_s,e,v,{"solver_version":"0.5.0","model_hash":model.model_hash(),"timestamp":datetime.now(timezone.utc).isoformat(),"legacy_bearing_speed_rule":"speed-dependent bearings use reference rotor speed, matching V2"})
def run_coaxial_frequency_response(model,speeds_rad_s,library_path=None):
    ...
    return CoaxialFrequencyResponseResult(sp,r,{"solver_version":"0.5.0","model_hash":model.model_hash(),"timestamp":datetime.now(timezone.utc).isoformat()})
