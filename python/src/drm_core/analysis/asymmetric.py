...
def run_asymmetric_modal(model,speed_rad_s,library_path=None,with_eigenvectors=False):
    ...
    return AsymmetricModalResult(speed_rad_s,e,v if with_eigenvectors else None,{"solver_version":"0.5.0","model_hash":model.model_hash(),"timestamp":datetime.now(timezone.utc).isoformat(),"legacy_chr_asym_nargout_semantics":True})
def run_asymmetric_frequency_response(model,speeds_rad_s,library_path=None):
    ...
    return AsymmetricFrequencyResponseResult(sp,r,{"solver_version":"0.5.0","model_hash":model.model_hash(),"timestamp":datetime.now(timezone.utc).isoformat()})
