...
def run_critical_speeds(model,library_path=None,**kwargs):
    original_options=dict(kwargs);want_diag=kwargs.pop('return_diagnostics',True);want_modes=kwargs.pop('with_mode_shapes',False);method=kwargs.get('method');facade=SolverFacade(library_path)
    if want_diag and (method is not None or 'initial_estimates' in kwargs):r,it,cv=facade.critical_speeds(model,return_diagnostics=True,**kwargs)
    else:r=facade.critical_speeds(model,**kwargs);it=cv=None
    modes=None
    if want_modes:
    ...
    return CriticalSpeedResult(np.asarray(r),None if it is None else np.asarray(it),None if cv is None else np.asarray(cv),modes,result_metadata(model,'crit_spd',original_options))
