...
for i = 1:nshaft
   ...
   if (Shaft_Type>0.5) & (Shaft_Type<8.5)
      n1 = Shaft_Def(i,2);n2 = Shaft_Def(i,3);
      dof = [4*n1-3:4*n1 4*n2-3:4*n2];
      Le = Node_Def(n2,2) - Node_Def(n1,2);
      outer_diameter = Shaft_Def(i,4);inner_diameter = Shaft_Def(i,5);
      rho = Shaft_Def(i,6);E = Shaft_Def(i,7);
      if ncol_shaft < 8,G = 0;else,G = Shaft_Def(i,8);end
      if ncol_shaft < 9,damping_factor = 0;else,damping_factor = Shaft_Def(i,9);end
      if ncol_shaft < 10,Axial_Force = 0;else,Axial_Force = Shaft_Def(i,10);end
      if ncol_shaft < 11,Torque = 0;else,Torque = Shaft_Def(i,11);end
      [M0e,C1e,K0e,K1e] = shftelem(Shaft_Type,Le,outer_diameter,inner_diameter,E,G,rho,Axial_Force,Torque);
   ...
   elseif (Shaft_Type>20.5) & (Shaft_Type<28.5)
      n1 = Shaft_Def(i,2);n2 = Shaft_Def(i,3);
      dof = [4*n1-3:4*n1 4*n2-3:4*n2];
      Le = Node_Def(n2,2) - Node_Def(n1,2);
      out_dia_1 = Shaft_Def(i,4);out_dia_2 = Shaft_Def(i,5);
      inn_dia_1 = Shaft_Def(i,6);inn_dia_2 = Shaft_Def(i,7);
      rho = Shaft_Def(i,8);E = Shaft_Def(i,9);
      if ncol_shaft < 10,G = 0;else,G = Shaft_Def(i,10);end
      if ncol_shaft < 11,Axial_Force = 0;else,Axial_Force = Shaft_Def(i,11);end
      [M0e,C1e,K0e,K1e] = taper(Shaft_Type,Le,out_dia_1,out_dia_2,inn_dia_1,inn_dia_2,E,G,rho,Axial_Force);
      M0(dof,dof) = M0(dof,dof) + M0e;
      C1(dof,dof) = C1(dof,dof) + C1e;
      K0(dof,dof) = K0(dof,dof) + K0e;
   else
...
for i = 1:ndisc
   ...
   if Disk_Type == 1 | Disk_Type == 3
      n1 = Disc_Def(i,2);rho = Disc_Def(i,3);thickness = Disc_Def(i,4);outer_diameter = Disc_Def(i,5);
      if ncol_disc == 6,inner_diameter = Disc_Def(i,6);else,inner_diameter = 0;end
   ...
   if Disk_Type == 2 | Disk_Type == 4
      n1 = Disc_Def(i,2);Mdisc = Disc_Def(i,3);Id = Disc_Def(i,4);
