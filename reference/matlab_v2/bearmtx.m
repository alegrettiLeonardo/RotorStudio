...
for i = 1:nbearing
   ...
   if Bearing_Type == 7       % fluid film bearings - based on short width bearing theory
      ...
      if ncol_bearing > 7  % if we have a nonlinear bearing don't include stiffness/damping here
          if Bearing_Def(i,8) ~= 0, 
              Kb1 = zeros(4,4); Cb1 = zeros(4,4);
          end
      end
   ...
   if Bearing_Type == 8       % seal
      ...
      Kb1(1:2,1:2) = epsilon*(mu_0-mu_2*T^2*Rotor_Spd^2/4)*eye(2,2) + epsilon*(mu_1*T*Rotor_Spd/2)*[0 -1; 1 0];
      Cb1(1:2,1:2) = epsilon*mu_1*T*eye(2,2) + epsilon*(mu_2*T^2*Rotor_Spd)*[0 -1; 1 0];
